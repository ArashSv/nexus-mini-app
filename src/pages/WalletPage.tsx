...
import { Wallet, ArrowDownLeft, ArrowUpRight, History, Zap } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="py-8 md:py-10 lg:py-12 space-y-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-slate-400 text-sm">Convert your efforts into digital assets.</p>
        </div>
        <GlassCard className="p-6 bg-gradient-to-br from-blue-600/20 to-purple-600/20 border-white/20">
          ...
          <div className="flex flex-col gap-4 pt-4 border-t border-white/10">
            <div className="flex items-center justify-between">
              <span className="text-slate-400 text-xs font-bold uppercase tracking-tighter">TON Wallet</span>
              <TonConnectButton />
            </div>
          </div>
        </GlassCard>
        <div className="grid grid-cols-2 gap-4">
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="outline" className="h-16 rounded-2xl bg-white/5 border-white/10 flex flex-col gap-1" onClick={() => hapticFeedback.impact('light')}>
                <ArrowDownLeft className="w-5 h-5 text-green-400" />
                <span className="text-xs font-bold">Deposit</span>
              </Button>
            </DialogTrigger>
            <DialogContent className="bg-slate-900 border-white/10 text-white max-w-[90%] rounded-3xl">
              <DialogHeader><DialogTitle className="text-center">Deposit NEX</DialogTitle></DialogHeader>
              <div className="flex flex-col items-center gap-6 py-4">
                <div className="p-4 bg-white rounded-2xl"><QRCode value={address || 'No Wallet Connected'} size={180} /></div>
                <p className="text-center text-xs text-slate-400 px-4">Transfer TON or NEX to this address to top up your balance.</p>
                <Button className="w-full h-12 rounded-xl bg-blue-600 font-bold" onClick={() => { if(address) { navigator.clipboard.writeText(address); toast.success("Copied!"); }}}>Copy Address</Button>
              </div>
            </DialogContent>
          </Dialog>
          <Dialog>
            ...
            <DialogContent className="bg-slate-900 border-white/10 text-white max-w-[90%] rounded-3xl">
              <DialogHeader><DialogTitle>Withdraw NEX</DialogTitle></DialogHeader>
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase">Destination TON Address</label>
                ...
                <Button className="w-full h-12 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 font-bold" onClick={handleWithdraw}>Request Withdrawal</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>
        <div className="space-y-4 pb-20">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-bold uppercase tracking-widest text-slate-400">Transaction History</h2>
          </div>
          <div className="space-y-2">
              ...
              <GlassCard key={tx.id} className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${tx.type === 'in' ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
                    {tx.type === 'in' ? <ArrowDownLeft className="w-4 h-4 text-green-400" /> : <ArrowUpRight className="w-4 h-4 text-red-400" />}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-white uppercase">{tx.type === 'in' ? 'Received' : 'Sent'}</p>
                    <p className="text-[10px] text-slate-500">{format(tx.timestamp, 'MMM dd, HH:mm')}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className={`text-sm font-black ${tx.type === 'in' ? 'text-green-400' : 'text-blue-400'}`}>
                    {tx.type === 'in' ? '+' : '-'}{tx.amountNEX} NEX
                  </p>
